...
load_image_registry cnpg-i-mcp-demo:${VERSION:-latest}

# Patch the deployment to use the provided image
kubectl patch deployments.apps -n cnpg-system cnpg-i-mcp-demo -p \
  "{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"cnpg-i-mcp-demo\",\"image\":\"${registry_name}:5000/cnpg-i-mcp-demo:${VERSION:-latest}\"}]}}}}"
